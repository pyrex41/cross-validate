...
16. **Argo `$values` multi-source resolution** — surfaces from replay-v4 (see `thoughts/shared/verify/replay-results-v4.md`). `ArgoSource` has no `Ref` field and the renderer passes `source.helm.valueFiles` paths starting with `$values/...` straight to `helm template`, which reads them as literal filesystem paths and fails. Scope: type + loader + renderer, plus fixture. Planning doc / vertical slice in `thoughts/shared/orchestration/xpc-values-multisource-handoff.md`. Expected replay-v5 delta: `XPC.H.helm-renders` 34 → ~13 (21 `$values` failures resolve; 12 local-chart `lib/charts/crossplane-*` remain by #15, plus 1 renovate pull protocol mismatch).
