...
1. ~~**Late-init-drift rule**~~ — ✅ shipped as R21 post-S5; registry has 7 rows across 3 kinds seeded from MRs !1048, !893, !1502. Report: `thoughts/shared/verify/r21-report.md`.
2. **R15 n×m cartesian FP flood** (P0, from replay) — R15 emits 64,484 errors from 288 unique (kind, group) pairs × ~224 Applications on the fg-manifold replay. Rule joins each resource against every Application rather than only the Application that actually owns the resource. Severe signal-to-noise. See `thoughts/shared/verify/replay-results.md`.
3. **XPC006 cartesian blowup** (P0, from replay) — same shape as R15: 30 unique messages × ~66 multiplier = 1,980 emissions. Likely same structural defect; a single fix may address both.
4. **Render cache not populating** (P1, from replay) — `~/.cache/xpc/` absent after 6 replay runs. Warm runs no faster than cold; S4 success criterion "hit rate >95% on second run" violated in production. Investigate `pkg/renderer/cache.go:55` disk-tier write.
5. **Remote Helm chart "Path required"** (P1, from replay) — `pkg/renderer/` rejects sources with `chart: X, repoURL: …, path: ""` with `unsupported source kind`. 34 render failures on fg-manifold tip 1 from this alone.
6. **xpc `--kernel-path` flag** (P2, from replay) — `resolveKernelPath` walks cwd upward only. Can't run xpc from outside its own repo tree; real CI unblocker. Workaround used during replay: `cd /path/to/cross-validate && xpc check /path/to/target/`.
