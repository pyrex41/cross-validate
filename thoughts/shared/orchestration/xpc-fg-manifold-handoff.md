...
15. **Missing fg-manifold `lib/charts/crossplane-*` paths** — triage only (xpc side verified). `ResolveChart` at `pkg/renderer/renderer.go:42` joins `src.Path` to `filepath.Dir(appFile)` verbatim; `substituteSource` does not rewrite `src.Path` beyond `{{ .key }}` substitution. No prefix-strip path for xpc to cause the error. Next action: user-side `ls` against the 3 replayed fg-manifold tips to confirm the charts are absent at those commits. If confirmed, close as repo-state artifact; if present, reopen with a pkg/ir investigation.
