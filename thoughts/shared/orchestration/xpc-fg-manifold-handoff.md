...
3. **XPC006 cartesian blowup** (P0, from replay) — same shape as R15 but on sync-wave / XRD / Composition / Function facts rather than resource-fact. Deferred: requires propagating `OwningApp` to `XRDInfo` / `CompositionInfo` / `FunctionInfo` / `ProviderInfo`, which is a structurally larger change. Still at 1,980 diagnostics per tip.
4. ~~**Render cache not populating**~~ — ✅ silent-failure fixed (`pkg/renderer/cache.go` now MkdirAlls eagerly, surfaces write errors, zeroes `DiskDir` on failure). Unit tests cover both success and read-only-parent paths. `~/.cache/xpc/renders/` now exists after first run. Disk hits cannot be observed on the fg-manifold workload because all its Helm sources are remote charts — see #5.
5. **Remote Helm chart "Path required"** (P1, from replay) — `pkg/renderer/` rejects sources with `chart: X, repoURL: …, path: ""` with `unsupported source kind`. 34 render failures on fg-manifold tip 1 from this alone. Also blocks warm-cache perf measurement.
